...
public class UDPTrackerClient extends TrackerClient {
	...
	@Override
	public void announce(AnnounceRequestMessage.RequestEvent event, boolean inhibitEvents) throws AnnounceException {
		logger.info("Announcing{} to tracker with {}U/{}D/{}L bytes...", new Object[] { this.formatAnnounceEvent(event),
				this.torrent.getUploaded(), this.torrent.getDownloaded(), this.torrent.getLeft() });

		State state = State.CONNECT_REQUEST;
		int maxAttempts = AnnounceRequestMessage.RequestEvent.STOPPED.equals(event) ? UDP_MAX_TRIES_ON_STOPPED
				: UDP_MAX_TRIES;
		int attempts = -1;
		...
		try {
				...
				case ANNOUNCE_REQUEST:
					...
					try {
						this.handleTrackerAnnounceResponse(
								UDPTrackerMessage.UDPTrackerResponseMessage.parse(this.recv(attempts)), inhibitEvents);
			...
			throw new AnnounceException(
					"Error while announcing" + this.formatAnnounceEvent(event) + " to tracker: " + ioe.getMessage(),
					ioe);
		} catch (MessageValidationException mve) {
			throw new AnnounceException("Tracker message violates expected " + "protocol (" + mve.getMessage() + ")",
					mve);
		}
	}
	 ...
	 */
	@Override
	protected void handleTrackerAnnounceResponse(TrackerMessage message, boolean inhibitEvents)
	...
	private UDPAnnounceRequestMessage buildAnnounceRequest(AnnounceRequestMessage.RequestEvent event) {
		return UDPAnnounceRequestMessage.craft(this.connectionId, transactionId, this.torrent.getInfoHash(),
				this.peer.getPeerId().array(), this.torrent.getDownloaded(), this.torrent.getUploaded(),
				this.torrent.getLeft(), event, this.peer.getAddress(), 0,
				TrackerMessage.AnnounceRequestMessage.DEFAULT_NUM_WANT, this.peer.getPort());
	}
	...
	private void send(ByteBuffer data) {
		try {
			...
			logger.warn("Error sending datagram packet to tracker at {}: {}.", this.address, ioe.getMessage());
		}
	}
